...
typedef struct __jmp_buf_tag_s {
    ...
    #ifdef ANDROID
    sigset_t         __saved_mask;  // Bionic's sigset_t is already 64-bit
    #else
...
struct x64_user_fpregs_struct
{
  ...
  unsigned int		st_space[32];   /* 8*16 bytes, x87 reg rounded to 16bytes  */
...
typedef struct my_x64_user_s
{
  struct x64_user_regs_struct	regs;
  int				u_fpvalid;
  struct x64_user_fpregs_struct	i387;
